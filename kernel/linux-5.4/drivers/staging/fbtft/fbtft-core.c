...
/* Logo support */
#ifdef CONFIG_LOGO
extern int fb_prepare_logo(struct fb_info *info, int rotate);
...
static int fbtft_fb_blank(int blank, struct fb_info *info)
{
	...
	/* 
	 * For small SPI displays, disable blanking to prevent
	 * unresponsive black screen issues. Always keep display on.
	 */
	switch (blank) {
	...
	case FB_BLANK_NORMAL:
		/* Don't blank - just call unblank to ensure display stays on */
		dev_info(info->dev, "Blank request ignored, keeping display on\n");
...
int fbtft_register_framebuffer(struct fb_info *fb_info)
{
	...
	/* Display boot logo on this framebuffer */
#ifdef CONFIG_LOGO
	dev_info(fb_info->dev, "Preparing to show boot logo...\n");
	fb_prepare_logo(fb_info, FB_ROTATE_UR);
	fb_show_logo(fb_info, FB_ROTATE_UR);
	dev_info(fb_info->dev, "Boot logo displayed\n");
#endif
