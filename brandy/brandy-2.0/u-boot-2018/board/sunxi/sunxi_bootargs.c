...
void update_bootargs(void)
{
	...
	gpio_init.port = 3;
	gpio_init.port_num = 7;
	gpio_init.mul_sel = 0;
	gpio_init.pull = 3;
	gpio_init.drv_level = -1;
	gpio_init.data = 1;
		
	nodeoffset = sunxi_gpio_request(&gpio_init, 1);
	gipista[1] = gpio_read_one_pin_value(nodeoffset,"board_gpio1");
	if(gipista[1] !=0)reset_misc();
	...
	sprintf(gipistastr," c5copen=%01d",gipista[3]);
	strcat(cmdline, gipistastr);
